...
@app.route(Config.REDIRECT_PATH)  # Must match Azure AD redirect URI
def authorized():
    ...
    if request.args.get('code'):
        ...
        if result is None or "error" in result:
            app.logger.warning("Failed MS login attempt")
            return render_template("auth_error.html", result=result)

        session["user"] = result.get("id_token_claims")
        # Use admin user for MS login
        user = User.query.filter_by(username="admin").first()
        login_user(user)
        app.logger.info("Successful MS login: admin")
        _save_cache(cache)
...
def _load_cache():
    cache = None
    # TODO: load cache from session if exists
    return cache
